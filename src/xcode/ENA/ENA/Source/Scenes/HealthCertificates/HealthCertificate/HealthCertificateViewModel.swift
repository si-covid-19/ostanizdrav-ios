...
final class HealthCertificateViewModel {
	...
	var additionalInfoCellViewModels: [HealthCertificateSimpleTextCellViewModel] {
		return [
			...
			HealthCertificateSimpleTextCellViewModel(
				backgroundColor: .enaColor(for: .cellBackground2),
				textAlignment: .left,
				text: "To potrdilo ni potna listina. Znanstveni dokazi o cepljenju, testiranju in okrevanju po covidu-19 se še naprej razvijajo tudi glede na nove različice virusa. Pred potovanjem preverite veljavne ukrepe glede javnega zdravja in z njimi povezane omejitve, ki veljajo v ciljni državi. Informacije o trenutnih omejitvah potovanja, ki veljajo v državah EU, so na voljo na https://reopen.europa.eu/sl",
				topSpace: 16.0,
				font: .enaFont(for: .body),
				borderColor: .enaColor(for: .hairline),
				accessibilityTraits: .staticText
	...
	private var nameAndDateOfBirthCellViewModel: [HealthCertificateKeyValueCellViewModel] {
		return [
			...
			HealthCertificateKeyValueCellViewModel(
				key: "Standardizirana priimek, ime",
				value: healthCertificate.name.reversedStandardizedName
			),
			HealthCertificateKeyValueCellViewModel(
				key: "Datum rojstva (YYYY-MM-DD)",
