...
class AppInformationImprintViewModel {
	...
	init(preferredLocalization: String = "sl") {
	...
	static func contactForms(localization: String) -> [DynamicCell] {
		...
		if localization == "en" || localization == "sl" { return [form] }
		let englishTitle: String = AppStrings.AppInformation.imprintSectionContactFormTitle + " " + "(English)"
		let germanTitle: String = AppStrings.AppInformation.imprintSectionContactFormTitle + " " + "(Slovenian)"
	...
	func initTable (localization: String = Bundle.main.preferredLocalizations.first ?? "sl") {
