...
private extension Client {
	...
	private func availableDaysAndHours(
		...
		group.enter()
		availableDays(forCountry: "SI") { result in
		...
		group.enter()
		wifiClient.availableHours(day: .formattedToday(), country: "SI") { result in
	...
	func fetchAllKeys(
		...
		availableDaysAndHours(
			...
			completion: { daysAndHours in
				self.fetchDays(daysAndHours.days, forCountry: "SI") { daysResult in
					wifiClient.fetchHours(daysAndHours.hours, day: .formattedToday(), country: "SI") { hoursResult in
