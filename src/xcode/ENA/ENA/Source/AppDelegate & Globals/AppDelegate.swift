	...
	private lazy var healthCertificateValidationService: HealthCertificateValidationProviding = {
		...
		if isUITesting {
			...
			if LaunchArguments.healthCertificate.invalidCertificateCheck.boolValue {
				...
				fakeResult.rule?.description = [Description(lang: "sl", desc: "Die Impfreihe muss vollständig sein (z.B. 1/1, 2/2)."), Description(lang: "en", desc: "The vaccination schedule must be complete (e.g., 1/1, 2/2).")]
