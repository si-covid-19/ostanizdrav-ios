...
extension ExposureDetectionViewController {
	...
	struct State {
		...
		var riskBackgroundColor: UIColor {
			!isTracingEnabled || riskDetectionFailed ? .enaColor(for: .background) : ((riskDetails?.minimumDistinctEncountersWithCurrentRiskLevel ?? 0) > 0 && riskLevel == .low) ? .enaColor(for: .riskMedium) : riskLevel.backgroundColor
		}

		var riskTintColor: UIColor {
			!isTracingEnabled || riskDetectionFailed ? .enaColor(for: .riskNeutral) : riskBackgroundColor
		}
