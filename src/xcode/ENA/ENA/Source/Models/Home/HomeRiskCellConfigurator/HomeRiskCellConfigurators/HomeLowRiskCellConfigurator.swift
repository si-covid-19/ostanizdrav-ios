...
final class HomeLowRiskCellConfigurator: HomeRiskLevelCellConfigurator {
	...
	private var color: UIColor = .enaColor(for: .riskLow)
	private let separatorColor: UIColor = .enaColor(for: .hairlineContrast)
	
	private var minimumDistinctEncountersWithCurrentRiskLevel: Int? = nil
	...
	init(
		...
		minimumDistinctEncountersWithCurrentRiskLevel: Int?
	) {
		self.numberOfDaysWithLowRisk = numberOfDaysWithLowRisk
		self.activeTracing = activeTracing
		self.minimumDistinctEncountersWithCurrentRiskLevel = minimumDistinctEncountersWithCurrentRiskLevel
		if let encounters = minimumDistinctEncountersWithCurrentRiskLevel {
			self.color = encounters > 0 ? .enaColor(for: .riskMedium) : .enaColor(for: .riskLow)
		}
